...
process_grid_element <- function(i,country,path.to.extdata,path.to.temdata,Tmaxdata,Tmindata,Sraddata,Rainfalldata,RelativeHum,coords,Soil,AOI) {
	...
	if (AOI) {
		Rainfalldata <- tidyr::pivot_longer(Rainfalldata, 
			 cols=-1:-7,
		...
		Sraddata <- tidyr::pivot_longer(Sraddata, 
					cols=-1:-7,
					names_to = c("Variable", "Date"),	
					names_sep = "_",	
					values_to = "SRAD") 
		...
		Tmaxdata <- tidyr::pivot_longer(Tmaxdata, 
					cols=-1:-7,
					names_to = c("Variable", "Date"),	
					names_sep = "_",	
					values_to = "TMAX") 
		...
		Tmindata <- tidyr::pivot_longer(Tmindata, 
						 cols=-1:-7,
						 names_to = c("Variable", "Date"),	
						 names_sep = "_",	
						 values_to = "TMIN") 
		...
		RelativeHum <- tidyr::pivot_longer(RelativeHum, 
					cols=-1:-7,
					names_to = c("Variable", "Date"),	
					names_sep = "_",	
					values_to = "RHUM") 
	...
	tst <- stats::na.omit(merge(Tmaxdata, merge(Tmindata,merge(Sraddata,merge(Rainfalldata,RelativeHum)))))
...
readGeo_CM <- function(country, useCaseName, Crop, AOI = FALSE, season=1) {
	...
	print("Progress:")
	results <- purrr::map(seq_along(grid[,1]), process_grid_element, country=country, path.to.extdata=path.to.extdata,
								 path.to.temdata=path.to.temdata, Tmaxdata=TemperatureMax, Tmindata=TemperatureMin, Sraddata=SolarRadiation,
								 Rainfalldata=Rainfall, RelativeHum=RelativeHum, coords=coords, Soil=Soil, AOI=AOI)
								# %||% print("Progress:")
}
